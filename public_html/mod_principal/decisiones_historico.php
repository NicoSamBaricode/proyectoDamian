...
try {
    ...
    if (!empty($ejercicio)) {
        $query = "SELECT ejercicio, fecha, precio_de_venta, unidades_a_producir, gastos_de_comercializacion, compra_bienes_de_uso, innovacion_desarrollo, sustentabilidad 
                  FROM decisiones 
                  WHERE empresa = ? AND ejercicio = ? 
                  ORDER BY fecha DESC";
        $stmt = $pdo->prepare($query);
        $stmt->execute([$usuario, $ejercicio]);
    } else {
        $query = "SELECT ejercicio, fecha, precio_de_venta, unidades_a_producir, gastos_de_comercializacion, compra_bienes_de_uso, innovacion_desarrollo, sustentabilidad 
                  FROM decisiones 
                  WHERE empresa = ? 
    ...
    error_log("Decisiones historico error: " . $e->getMessage());
    $recordset = [];
}
      ...
      <tr>
        <td bgcolor="#D3DBE2"><table width="770" border="0" cellspacing="1">
          <tr>
            ...
            <td bgcolor="#A8B6C6" class="etiquetas">Gastos de comercialización</td>
            ...
            <td bgcolor="#A8B6C6" class="etiquetas">Innovación y Sustentabilidad</td>
          </tr>
          <?php 
          $color_fondo = "#FFFFFF";
